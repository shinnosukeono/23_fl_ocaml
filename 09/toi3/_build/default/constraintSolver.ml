...
let rec ty_subst subst ty =
  ...
  | TyPair(p1,p2) ->
    TyPair((ty_subst subst p1),(ty_subst subst p2))
  | TyCons(p1,p2) ->
    TyCons((ty_subst subst p1),(ty_subst subst p2))
...
let rec compose subst_f subst_g =
  ...
  |(tyvar,ty)::subst ->
    match ty with
    |TyInt | TyBool | TyFun(_,_) | TyPair(_,_) | TyCons(_,_) -> 
...
let rec unify constraints =
  ...
  |(ty1,ty2)::constraints'->
    (match ty1,ty2 with
      ...
      | (TyPair(t11,t12),TyPair(t21,t22)) -> (unify([(t11,t21);(t21,t22)]@constraints'))
      | (TyCons(t1),TyCons(t2)) -> (unify([(t1,t2)]@constraints'))
      | _,_ ->raise TyError)
